...
// validateNestedStruct recursively validates a nested struct's fields against schema attributes
// This supports Go native structs used in nested list/map/set items
func validateNestedStruct(t *testing.T, structType reflect.Type, schemaAttrs map[string]schema.Attribute, path string) {
	...
	// Validate each field in the nested struct
	...
	// Check for schema attributes without corresponding model fields
	for attrName := range schemaAttrs {
		found := false
...
// validateNestedStructDataSource recursively validates a nested struct's fields against data source schema attributes
...
// validateElementType checks if a schema attribute's ElementType matches the expected type
func validateElementType(t *testing.T, schemaElementType attr.Type, expectedKind reflect.Kind, attrPath string) {
	t.Helper()

	// Map expected kinds to their corresponding types.Type
	switch expectedKind {
	case reflect.String:
...
func validateObjectTypeAgainstSchema(t *testing.T, nestedAttrs map[string]schema.Attribute, path string) {
	...
	for attrName, schemaAttr := range nestedAttrs {
		...
		switch attr := schemaAttr.(type) {
		case schema.ListAttribute:
			// Check that ListAttribute has a valid ElementType
			validateSchemaElementType(t, attr.ElementType, attrPath)

		case schema.MapAttribute:
			// Check that MapAttribute has a valid ElementType
			validateSchemaElementType(t, attr.ElementType, attrPath)

		case schema.SetAttribute:
			// Check that SetAttribute has a valid ElementType
			validateSchemaElementType(t, attr.ElementType, attrPath)
		...
		case schema.ListNestedAttribute:
			// Recursively validate nested object attributes
...
// validateSchemaElementType checks if a schema's ElementType is valid and consistent
func validateSchemaElementType(t *testing.T, elemType attr.Type, attrPath string) {
	t.Helper()

	// We can at least verify that ElementType is not nil and is a valid type
	if elemType == nil {
...
func ValidateSchemaAgainstObjectType(t *testing.T, schemaAttrs map[string]schema.Attribute, attrTypes map[string]attr.Type, path string) {
	...
	for attrName, expectedType := range attrTypes {
		...
		// Check if the schema attribute type matches the expected attr.Type
		switch expectedType {
		case types.StringType:
			// Verify schema is StringAttribute
		...
		case types.BoolType:
			// Verify schema is BoolAttribute
		...
		case types.Int64Type:
			// Verify schema is Int64Attribute
		...
		case types.Int32Type:
			// Verify schema is Int32Attribute
		...
		case types.Float64Type:
			// Verify schema is Float64Attribute
		...
		case types.NumberType:
			// Verify schema is NumberAttribute
		...
		default:
			// Handle typed cases (ListType, MapType, SetType, ObjectType)
			...
			case types.MapType:
				// Verify schema is MapAttribute and ElementType matches
			...
			case types.SetType:
				// Verify schema is SetAttribute and ElementType matches
