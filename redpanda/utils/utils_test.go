	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedErr == nil {
				...
				// Handle specific error type comparisons
				if timeoutErr, ok := tc.expectedErr.(*TimeoutError); ok {
					// For timeout errors, verify the type and properties
					actualTimeoutErr, ok := err.(*TimeoutError)
					assert.True(t, ok, "expected TimeoutError")
				...
				} else if notFoundErr, ok := tc.expectedErr.(NotFoundError); ok {
					// For NotFoundError, verify the type and message
					actualNotFoundErr, ok := err.(NotFoundError)
