# Private-only serverless fixture: a public-disabled serverless cluster is
# reachable only from inside the private link VPC, so Schema Registry (and the
# redpanda_schema_registry_acl / redpanda_schema resources that depend on it)
# cannot be exercised from an external acc-test runner. This fixture covers the
# control-plane + Kafka-dataplane lifecycle only; SR coverage lives in the
# public and "both" variants.
