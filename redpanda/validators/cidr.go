...
func (v CIDRBlockValidator) Description(_ context.Context) string {
	return "ensures that cidr_block is set only when customer_managed_resources is not set"
}

func (v CIDRBlockValidator) MarkdownDescription(_ context.Context) string {
	return "Ensures that `cidr_block` is set only when `customer_managed_resources` is not set"
}

func (v CIDRBlockValidator) ValidateString(ctx context.Context, req validator.StringRequest, resp *validator.StringResponse) {
