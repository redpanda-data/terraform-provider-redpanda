...
// Network sweeps a Redpanda Cloud network by name.
type Network struct {
	NetworkName string
	Client      *cloud.ControlPlaneClientSet
}

// SweepNetworks deletes the network matching NetworkName.
func (s Network) SweepNetworks(_ string) error {
	...
	op, err := s.Client.Network.DeleteNetwork(ctx, &controlplanev1.DeleteNetworkRequest{
		Id: network.GetId(),
	})
	if err != nil {
