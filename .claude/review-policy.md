...
Judgment policy for code review in this repo, whether run interactively (`/review`, the `reviewer` agent) or by a CI reviewer. The rules a change is judged against live in `CLAUDE.md` and the skills; this file says only how to review: what clears the bar, what to filter, how to report.
...
If you are not certain an issue is real, do not flag it. Verify against the proto in `../cloudv2` or `../console` before flagging a writability claim. Before flagging plan or apply behavior, show a config that reaches it and survives the validators that run first; a scoped `go test -run` that reproduces it settles the question.
