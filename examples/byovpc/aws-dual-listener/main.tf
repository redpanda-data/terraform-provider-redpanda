...
# public_subnets is preview and feature-gated (enable-public-private-listeners).
# It can be added to an existing network in place; once set it cannot be
# changed or removed without recreating the network.
resource "redpanda_network" "test" {
  ...
  customer_managed_resources = {
    aws = {
      ...
      public_subnets = {
        arns = module.redpanda_byovpc.public_subnet_arns
      }
    }
  }
  depends_on = [
...
# Dual listener mode: connections on all three services with the same
# public/private topology, and no connection_type.
