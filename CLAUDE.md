...
- **Actions live in `redpanda/actions/<name>/` and are hand-written.** schemagen has no action support; an action's schema, tests, `templates/actions/` page, and `examples/docs/<name>/` example are all maintained by hand. Actions need Terraform 1.14, so a live lane exercises one through a temp-dir overlay of its config directory rather than in the config the upgrade entry applies with the released provider (see `redpanda/tests/runner_byoc_agent_test.go`).
- **Two test trees, split by tier, not by resource.** `redpanda/resources/<r>/integration_*_test.go` (package `<r>_test`, build tag `integration`) is the in-process bufconn tier against `internal/testutil/mock/fakes/`. `redpanda/tests/` is live-only. Shared harness code is `internal/testutil/{acc,integration,mock}/`.
