...
- `redpanda/validators/` — custom plan/config validators (cidr, cpu_shares, cluster_configuration, …); named-validator house style
- `redpanda/kclients/` — Kafka-API / schema-registry clients for dataplane resources
- `redpanda/base/`, `redpanda/config/` — resource base helpers and provider config
...
- `redpanda/utils/` — shared helpers (retry, backoff, etc.); `redpanda/utils/enums/` holds the generated enum mappers
...
- `internal/{apidesc,bufdeps,clustermask,cmdutil,fileutil,modelconv,provider}/` — codegen + provider support libs (proto descriptors, buf-dep pinning, cluster mask paths, shared cmd helpers, file utils, model conversion, provider plumbing)
