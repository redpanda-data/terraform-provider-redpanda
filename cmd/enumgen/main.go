...
func emitFile(path string, enums map[string]enumInfo) error {
	...
	b.WriteString(fileutil.LicenseHeader())
	b.WriteString(`// Code generated by cmd/enumgen. DO NOT EDIT.
...
`)
	...
	for _, k := range keys {
		...
		b.WriteString("// Returns the UNSPECIFIED value for unknown inputs.\n")
	...
	if err != nil {
		if writeErr := os.WriteFile(path, fileutil.PreserveHeaderYear(path, []byte(b.String())), 0o600); writeErr != nil {
	...
	if err := os.WriteFile(path, fileutil.PreserveHeaderYear(path, src), 0o600); err != nil {
