...
func TestRender(t *testing.T) {
	...
	truncated := Render(Report{PR: r.PR, Threads: []ReviewThread{{Path: "y.go", Author: "reviewer", Body: "long one", CommentsTruncated: true}}})
	if !strings.Contains(truncated, "more comments than shown") {
		t.Errorf("truncated thread must say its comments were cut:\n%s", truncated)
	}
