...
type ReviewThread struct {
	...
	Body   string
	URL    string
	// The comment list hit the per-thread cap; later comments are only at URL.
...
// read selectively instead of dumped into context. The .logs/ gitignore rule
// keeps them out of git; the suffix only marks them as disposable output.
...
func Render(r Report) string {
	...
	for _, t := range r.Threads {
		...
		if t.CommentsTruncated {
			b.WriteString("  (thread has more comments than shown; read it at the link)\n")
		}
		if t.URL != "" {
			fmt.Fprintf(&b, "  %s\n", t.URL)
		}
