...
// A thread that returns exactly commentsPerThread comments is reported as
// truncated rather than paged: a 50-comment thread is rare enough that the
// link is the better tool.
const commentsPerThread = 50
...
func fetchThreads(ctx context.Context, number int) ([]ReviewThread, error) {
	...
	return collectThreads(func(cursor string) ([]byte, error) {
		args := []string{"api", "graphql", "-f", "query=" + threadsQuery, "-F", "owner=" + owner, "-F", "repo=" + repo, "-F", "pr=" + strconv.Itoa(number)}
		if cursor != "" {
...
// collectThreads walks every page of review threads; page fetches one page
// starting after cursor ("" for the first).
func collectThreads(page func(cursor string) ([]byte, error)) ([]ReviewThread, error) {
	...
	for {
		...
		for _, n := range rt.Nodes {
			...
			threads = append(threads, ReviewThread{
				Path: n.Path, Line: n.Line, Author: first.Author.Login, Body: first.Body, URL: first.URL,
				CommentsTruncated: len(n.Comments.Nodes) >= commentsPerThread,
			})
		}
